...
public class FileUtil {
    ...
    public static void upload(MultipartFile multipartFile, String directoryPath, String fileName) {
        ...
        Path pathToDirectory = Path.of(directoryPath);
        String separator = FileSystems.getDefault().getSeparator();
        Path fileDist = Path.of(directoryPath + separator + fileName);

        try {
            if (!exists(pathToDirectory)) {
                createDirectories(pathToDirectory);
            }
            copy(multipartFile.getInputStream(), createFile(fileDist));
        } catch (IOException e) {
